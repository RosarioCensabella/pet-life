...
void main() {
  ...
  testWidgets('User can add and delete an expense entry', (
    tester,
  ) async {
    ...
    expect(find.text('Nessuna spesa'), findsOneWidget);
    expect(find.textContaining('consulenza fiscale'), findsOneWidget);
    expect(find.textContaining('Nessuna spesa registrata'), findsOneWidget);
    ...
    await tester.enterText(find.byType(TextFormField).at(2), 'Clinica Pet Life');
    await tester.enterText(find.byType(TextFormField).at(3), 'Ricevuta salvata');

    await tester.ensureVisible(find.text('Salva spesa'));
    await tester.pump(const Duration(milliseconds: 300));

    await tester.tap(find.text('Salva spesa'));
    await tester.pumpAndSettle();

    expect(find.text('Visita veterinaria'), findsOneWidget);
    expect(find.text('49.90 EUR'), findsWidgets);
    expect(find.textContaining('Clinica Pet Life'), findsOneWidget);
    expect(find.text('Ricevuta salvata'), findsOneWidget);

    await tester.tap(find.byIcon(Icons.delete_outline).first);
    await tester.pumpAndSettle();

    expect(find.text('Eliminare questa spesa?'), findsOneWidget);
